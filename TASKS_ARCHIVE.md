...
- 2026-07-22: Promovido a `TASKS.md`, `scope` cerrado con blueprint aprobado por Marco. Fusionado con **T166** (backlog, baseline físico pendiente en onboarding) — mismo patrón de UI, ya desbloqueado por T152. Blueprint: Parte 1 (flags en-app del coach en `Equipo.jsx`/`AlumnoDetalle.jsx`, mismo patrón que "Reclutamiento pendiente") + Parte 2 (digest semanal por email vía Resend). Fuera de alcance confirmado por Marco: notificar directo a papás, expandir a atletas, 2FA/WhatsApp (T043).
- 2026-07-22: `design` cerrado — tag ámbar reusado (`#FFF6D6`/`#8A6D00`), máximo 2 badges + "+N más" en vista lista (`AthleteCard`/`TeamTable`), desglose completo sin cap en `AlumnoDetalle.jsx`. Orden de prioridad: Perfil incompleto → PTF pendiente → Baseline físico pendiente → Papás/tutor pendiente → Reclutamiento pendiente.
- 2026-07-22: `build` de Parte 1 completo. `onboardingGaps()` en `src/lib/athletics.js` como single source of truth de los 5 gaps, reusada por `Inicio.jsx` (refactor sin cambio de comportamiento), `Equipo.jsx` y `AlumnoDetalle.jsx`. Fix de correctness encontrado: "Reclutamiento pendiente" pasó de chequear solo existencia de fila a completitud real de campos. `GapBadges` (componente compartido nuevo). 22 tests nuevos, 149/149 en verde.
- 2026-07-22: `verify-ui` — walkthrough real en sandbox como coach. Confirmado en vivo: flags correctos en Equipo → AlumnoDetalle, cap de 2 + "+N más" funcionando. Bug real encontrado en el camino (RLS heredado de T140, ver **T168**) — fix aplicado y verificado antes/después. Mobile no verificable con la herramienta de browser automation (mismo gap ya documentado en T152) — código revisado, reusa patrones responsive existentes sin clases nuevas. Status → `in review`.
- 2026-07-22: Gap de seguridad encontrado y cerrado en el camino (bloqueaba probar Parte 2 localmente): `.env.local` tenía la service key de **producción** guardada bajo el nombre sin sufijo `SUPABASE_SERVICE_KEY`, emparejada por error con la URL del sandbox. Confirmado con una lectura mínima de solo lectura contra ambos proyectos. Marco confirmó el reemplazo por la key correcta del sandbox; `.env.local`/`.env.example` reorganizados según la convención de sufijos ya documentada pero nunca aplicada (`SUPABASE_URL_SANDBOX`/`SUPABASE_SERVICE_KEY_SANDBOX` para uso local rutinario; `SUPABASE_URL`/`SUPABASE_SERVICE_KEY` sin sufijo quedan vacíos, reservados para uso transitorio deliberado contra producción).
- 2026-07-22: Parte 2 (digest semanal por email) construida. `filterGapsForDigest()` nuevo en `athletics.js` (gracia de `ONBOARDING_GRACE_DAYS`/`PTF_GRACE_DAYS`). `scripts/onboarding_digest.mjs` (Node/ESM, reusa `onboardingGaps` directo de `athletics.js`) + `.github/workflows/onboarding-digest-cron.yml` (cron lunes 7am CDMX). Verificado con dry-run real contra sandbox, coincide con lo visto en `verify-ui`. Bug chico arreglado (`RESEND_FROM` con `??` en vez de `||`).
- 2026-07-22: Deploy roto encontrado y arreglado — un test nuevo de `onboardingGaps` dependía de timezone (fecha `-01-01`, mismo problema de parseo UTC-vs-local que `fmtPeriod` ya documentaba) y además tenía una aserción de edad incorrecta (16 años tratados como >=17). Producción no se vio afectada (Vercel nunca promovió el build fallido). Fix verificado determinístico en 3 timezones distintas. Gap de proceso identificado: revisar el status del deploy después de cada push de aquí en adelante.
- 2026-07-22: Marco confirmó la Parte 1 (flags de onboarding) — incluye la confirmación visual en mobile que quedaba pendiente. La Parte 2 (digest) se verifica aparte con un envío de prueba real.
...
**Notas:**
- 2026-07-22: Marco pidió revisar el cron de GH del scraper de AMTP (fallaba con exit code 1) y por separado reportó que "Ian Kleiman" aparecía duplicado en `/portal/equipo` en producción — investigados juntos porque el log del scraper mostraba el nombre duplicado en la lista de atletas de la plataforma.
  - **Duplicado de Ian Kleiman confirmado en `athletes` (producción)**: 2 filas — una con coach asignado, 2 `quarterly_plans` + 1 fila en `amtp_rankings` (datos reales); otra sin coach, cero datos en cualquier tabla (cuenta duplicada, dada de alta dos veces). Confirmado con Marco: desactivada (`activo=false`) la fila vacía, mismo patrón que T140 (nunca `DELETE`, baja reversible). No se tocó su `auth.users` asociado.
  - **Causa real del 409 del scraper — NO era el duplicado de Ian Kleiman.** Reproducido el batch upsert exacto (5 filas) directo por SQL con `ON CONFLICT (nombre, genero, periodo) DO UPDATE` — funcionó sin error. El bug real: `scripts/amtp_scraper.py` hacía `POST {base}/amtp_rankings` con `Prefer: resolution=merge-duplicates` pero sin el query param `on_conflict=nombre,genero,periodo` — sin eso, PostgREST resuelve el upsert contra la PK (`id`, que nunca choca porque el payload no la manda) y termina haciendo un INSERT plano que revienta contra la unique constraint real en cuanto ya existe una fila para ese período — fallaba en cualquier corrida que no fuera la primera del mes.
  - Fix: agregar `?on_conflict=nombre,genero,periodo` a la URL del POST. Commiteado (`aab3654`), pusheado, deploy verde. Verificado con corrida real del workflow (`workflow_dispatch`, run 29966601499): `✓ Upsert completo: 5 registros para período 2026-07`, sin errores.
...
**Notas:**
- 2026-07-22: Creado a partir de la deuda técnica identificada durante `build` de T152 (2026-07-21, ya `done`): `supabase/functions/generate-quarterly-plan/index.ts` seguía generando objetivos de `dimension='physical'` con el esquema viejo de sub-dimensiones (`sprint_20m`, `beep_test`, `fuerza_inferior`, etc.), no con el protocolo real de 7 pruebas RAC migrado en T152. Como parche temporal, `MiPlan.jsx`/`PlanesCoach.jsx`/`NuevoReporte.jsx` soportaban ambos esquemas de labels en paralelo.
- 2026-07-22: `scope` cerrado, blueprint aprobado por Marco. Hallazgos del código real: `SUB_DIMENSIONS.physical` (líneas 29-37) tenía las 7 keys viejas — había que reemplazarlas por las 7 reales de `report_physical` exactas, porque `monthlyScoresForFoco`/`physicalTestScore` (T152) leen `report_physical[sub_dimension]` directo. Dos ejemplos de calibración en `RUBRICA_OBSERVACIONES` referenciaban keys que desaparecerían (`fuerza_inferior`, `beep_test`). Línea 194 ("physical → BANDAS NUMÉRICAS alrededor del target") era texto aspiracional muerto — la función nunca recibe baseline/target/unit en el request. No hay sandbox de Edge Functions (`generate-quarterly-plan` solo existe en producción) — plan de verificación: deploy a slug temporal, pruebas sintéticas, deploy real. Riesgo aceptado: 4 objetivos reales existentes con keys viejas (`beep_test`×3, `fuerza_inferior`×1) dejan de pasar `ALL_KEYS`/`ALL_DIMS` en regenerate (fallback suave, bajo impacto, ya sin scoring posible desde T152). Marco identificó un gap conceptual más grande (el modelo de anclas por trimestre no encaja con el baseline fijo de physical) — deliberadamente fuera de este task, creado **T169** en `BACKLOG.md` (High) para resolverlo después.
- 2026-07-22: `build` + `verify-evals` completos. `SUB_DIMENSIONS.physical` reemplazadas por las 7 keys reales (`velocidad_2377m`, `agilidad_5_lineas_seg`, `abdominales_30s`, `salto_vertical_cm`, `lanzamiento_balon_mts`, `flexibilidad_banco_pass`, `tiempo_1km_seg`); 2 ejemplos de calibración reescritos con contenido narrativo equivalente; línea de anclas de `physical` reescrita para no prometer bandas numéricas que el código nunca entrega. `PROMPT_VERSION` → `pm-v4.1-2026-07-22`. `verify-evals`: deploy a `generate-quarterly-plan-test` en producción, 3 llamadas sintéticas reales vía `curl` — `identify` con dump de velocidad+resistencia aeróbica devolvió las keys RAC correctas con `candidata_a_foco`/`urgencia` sensatos; `generate` sobre ese foco produjo objetivo + anclas -2..+2 cualitativas sin cifras inventadas; `identify` con observación física vaga confirmó que el ejemplo de calibración reescrito sigue marcando `observacion_suficiente: false` correctamente. Deploy real a `generate-quarterly-plan` (v17→v18) confirmado con una 4ª llamada directa al slug real. Pendiente no bloqueante: el slug temporal `generate-quarterly-plan-test` sigue en producción (sin tooling MCP para borrarlo, inofensivo) — Marco lo borra a mano desde el dashboard de Supabase cuando pueda.

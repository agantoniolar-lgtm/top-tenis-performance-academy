...
| 2026-07-23 | `20260723000000_create_athlete_notes.sql` | Claude (MCP) | Sí — tabla nueva, sin datos existentes que romper. **Verificación activa por rol corrida de verdad** en sandbox (sembrando 2 `auth.users` de prueba con pgcrypto, ver T165): coachA-autor lee su nota ✓; coachB-otro-coach lee cross-coach ✓ pero UPDATE/DELETE ajenos → 0 filas ✓; coachB INSERT propia ✓ pero suplantando a otro coach → RLS 42501 ✓; atleta ve 0 notas y no puede insertar → RLS 42501 ✓. Sandbox limpiado después. `verify-ui` también caminado (crear/validar/borrar notas como coach) | Sí | T148 fase 1. Tabla `athlete_notes` (notas de voz/texto internas del coach) + 4 policies (SELECT cross-coach, INSERT/UPDATE/DELETE solo autor; atleta sin acceso). Post-aplicación en prod: RLS habilitado, 4 policies confirmadas, 0 filas. Columnas de voz (`kind`/`audio_path`/`transcription_*`) creadas desde ya para que la fase 2 no requiera un ALTER; fase 1 solo escribe `kind='text'`. |

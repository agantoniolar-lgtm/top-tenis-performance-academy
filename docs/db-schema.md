...
| `athlete_notes` | Lee de cualquier atleta (cross-coach); crea, edita **y borra** solo las que él mismo escribió (`coach_id` = autor). | **Sin acceso** | T148 (23 Jul 2026), migración `create_athlete_notes`. Notas de voz/texto internas del coach — mismo modelo cross-coach que `reports`, pero **con DELETE del autor** (a diferencia de `athletes`/`quarterly_plans`): una nota mal capturada debe poder eliminarse. RLS verificada en sandbox por rol (coachA-autor, coachB-ajeno, atleta) antes de UI. Fase 2a (migración `athlete_notes_audio`): columna `audio_duration_seconds` (numeric, solo notas de voz vía CHECK). |
| `storage.objects` bucket `athlete-notes-audio` (privado) | Sube y lee cualquier audio (cross-coach); actualiza/borra solo los que él subió (`owner = auth.uid()`). | **Sin acceso** | T148 fase 2a. Audio de las notas de voz. Bucket creado vía Storage API (como `public-media`); policies en migración `athlete_notes_audio`. DELETE/UPDATE por-owner no ejercitables por SQL directo (trigger `storage.protect_delete` obliga a usar la Storage API) — verificadas estructuralmente contra las policies de `public-media` ya en prod. |

...
import { FILE, buildShallowFile } from '../../data/athletes';
import { usePortal } from '../../contexts/PortalContext';
import RosterPanel from '../../components/portal/RosterPanel';
import {
  Icon, Eyebrow, Card, Btn, Tag, Tabs, ScoreRing, ScoreBar,
} from '../../components/portal/ui';

// ─── Expediente root ──────────────────────────────────────────────────────────
export default function Expediente() {
  const { id }           = useParams();
  const { role, athleteId } = usePortal();
  const [tab, setTab]    = useState('resumen');
  const [tcOpen, setTcOpen] = useState(false);

  const isStaff = role === 'coach' || role === 'admin';

  // Atleta/padre always see their own file
  const effectiveId = (role === 'atleta' || role === 'padre') ? FILE.id : (id || athleteId || FILE.id);
  const file = effectiveId === FILE.id ? FILE : (buildShallowFile(effectiveId) || FILE);
  ...
  return (
    <div className="flex flex-1 min-w-0">
      ...
      <div className="flex-1 min-w-0">
            ...
            <div className="grid grid-cols-12 gap-6 items-start">
              ...
              <div className="col-span-12 lg:col-span-5 flex gap-5">
                ...
                <div className="min-w-0">
                  ...
                  <h1 className="font-display font-extrabold text-[36px] leading-[0.95] tracking-[-0.02em]">
                    {file.nombre}
                  </h1>
              ...
              <div className="col-span-12 lg:col-span-2 flex lg:flex-col gap-2 lg:items-end">
                {!readOnly && (
                  <Btn variant="primary" size="md" icon="sparkle" onClick={() => setTcOpen(true)}>
                    Talent Card
                  </Btn>
                )}
                ...
                <div className="text-[10px] font-mono text-[var(--ink-mute)] uppercase text-right hidden lg:block">
        ...
        {/* Tab content */}
        <div className="p-6">
          {tab === 'resumen'     && <TabResumen     file={file} onTab={setTab} onOpenTC={() => setTcOpen(true)} />}
      ...
      {tcOpen && <TalentCardModal file={file} onClose={() => setTcOpen(false)} />}
    </div>
  );
}
...
// ─── RESUMEN tab ─────────────────────────────────────────────────────────────
function TabResumen({ file, onTab, onOpenTC }) {
  return (
    <div className="grid grid-cols-12 gap-4">
      {/* 5 dimensions */}
      <div className="col-span-12 lg:col-span-8">
        <Card
          label="Expediente · 5 dimensiones"
          title="Las 5 dimensiones que evalúa la academia"
          action={
            <div className="flex items-center gap-2">
              <Btn size="sm" variant="outline" icon="download">Exportar</Btn>
              <Btn size="sm" variant="primary" icon="sparkle" onClick={onOpenTC}>Talent Card</Btn>
            </div>
          }
          noPad
        >
          <div className="grid grid-cols-2 md:grid-cols-5">
            {file.dimensions.map((d, i) => (
              <button
                key={d.key}
                onClick={() => onTab(d.key)}
                className="flex flex-col items-center py-6 px-4 text-center relative group transition hover:bg-[var(--cream)]"
          ...
          <div className="px-5 py-3 hairline-t flex items-center justify-between text-[11px] font-mono uppercase">
            ...
            <div className="flex items-center gap-3">
              <span className="flex items-center gap-1.5"><span className="w-2 h-2 inline-block" style={{ background: 'var(--good)' }} /> 4.0–5.0</span>
              <span className="flex items-center gap-1.5"><span className="w-2 h-2 inline-block" style={{ background: 'var(--warn)' }} /> 3.0–3.9</span>
              <span className="flex items-center gap-1.5"><span className="w-2 h-2 inline-block" style={{ background: 'var(--bad)' }} /> &lt; 3.0</span>
            </div>
          </div>
        </Card>
      </div>
        ...
        <Card label="Perfil multidimensional" title="Composite">
          ...
          <div className="hairline-t pt-3 mt-2 flex items-center justify-between text-[11px]">
            <span className="flex items-center gap-1.5">
              <span className="w-3 h-1.5 inline-block" style={{ background: 'var(--accent)' }} />
              <span className="text-[var(--ink)] font-medium">Coach / staff</span>
            </span>
            <span className="flex items-center gap-1.5">
              <span className="w-3 h-1.5 inline-block border" style={{ borderColor: 'var(--ink)' }} />
      ...
      <div className="col-span-12 lg:col-span-8">
        <Card
          label="Progresión UTR · 18 meses"
          title={`UTR ${file.utr.now.toFixed(1)} / +${file.utr.delta12.toFixed(1)} vs hace 12m`}
          action={
            <div className="flex items-center gap-1">
              {['UTR', 'AMTP', 'ITF SJR'].map((p, i) => (
                <button
                  key={p}
                  className={`px-2.5 h-7 text-[11px] font-mono uppercase tracking-wider ${i === 0 ? 'text-white' : 'text-[var(--ink-mute)]'}`}
                  style={i === 0 ? { background: 'var(--ink)' } : {}}
                >
                  {p}
                </button>
              ))}
          ...
          {file.activity.map((a, i) => (
            <div key={i} className="flex items-start gap-3 px-5 py-3 hairline-b last:border-b-0">
              <div className="w-7 h-7 flex items-center justify-center shrink-0 mt-0.5"
                ...
                <Icon name={
                  a.type === 'video' ? 'play' : a.type === 'ptf' ? 'mic' :
                  a.type === 'session' ? 'pulse' : a.type === 'fitness' ? 'bolt' :
      ...
      <div className="col-span-12 lg:col-span-7">
        <Card
          label="Torneos recientes"
          title="Resultados · últimos 6"
              ...
              {file.tournaments.slice(0, 6).map(t => {
                ...
                return (
                  <tr key={t.id} className="hairline-t hover:bg-[var(--cream)] transition">
                    ...
                    <td className="px-3 py-3"><Tag bg="var(--cream)">{t.round}</Tag></td>
      ...
      <div className="col-span-12 lg:col-span-5">
        <Card
          label="05 · Athlete Voice · último PTF"
        ...
        >
          <div className="mb-3">
            ...
            <div className="text-[11px] font-mono text-[var(--ink-mute)] mt-1">
              {file.voice.latestPTF.fecha} · {file.voice.latestPTF.result}
            </div>
          </div>
          <div className="relative pl-4 mt-4 border-l-2" style={{ borderColor: 'var(--accent)' }}>
            <p className="text-[14px] leading-relaxed text-[var(--ink)] italic">
              &ldquo;{file.voice.latestPTF.reflection}&rdquo;
            </p>
          </div>
          <div className="mt-4 grid grid-cols-2 gap-3 text-[12px]">
            <div className="p-3" style={{ background: 'var(--cream)' }}>
  ...
  return (
    <div className="bg-[var(--paper)] px-4 py-3">
      ...
      <div className="flex items-baseline gap-2">
        ...
        {delta && (
          <div className="text-[10px] font-mono font-bold" style={{ color: good ? 'var(--good)' : 'var(--bad)' }}>
            {delta}
          </div>
        )}
      </div>
    ...
    <div className="relative">
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full">
          ...
          <g key={i} onMouseEnter={() => setHover(i)} onMouseLeave={() => setHover(null)}>
            ...
            {hover === i && (
              <g>
                ...
                <text x={x(i)} y={y(p.v) - 15} textAnchor="middle" fontSize="11"
                      fontFamily="Big Shoulders Display" fontWeight="900" fill="white">
                  {p.v.toFixed(1)}
                </text>
              </g>
            )}
        ...
        {points.filter((_, i) => i % 4 === 0).map((p, i) => (
          <text key={i} x={x(i * 4)} y={H - 8} fontSize="10" fontFamily="JetBrains Mono"
...
function Radar({ dimensions, self }) {
  ...
  const coachPts = dimensions.map((d, i) => point(i, d.score));
  const selfPts  = dimensions.map((d, i) => point(i, selfMap[d.label] ?? d.score));
    ...
    <div className="grid grid-cols-12 gap-4">
          ...
          <div className="space-y-5">
            {oc.strokes.map(s => (
              <div key={s.key} className="grid grid-cols-12 gap-4 items-start">
                ...
                <div className="col-span-9 md:col-span-4"><ScoreBar value={s.score} /></div>
            ...
            <div className="hairline-t pt-5">
              <div className="grid grid-cols-12 gap-4 items-start">
                ...
                <div className="col-span-9 md:col-span-4"><ScoreBar value={oc.tactics.score} /></div>
      ...
      <div className="col-span-12 lg:col-span-4">
        <Card label="Clips" title="Resumen de partido"
              action={<Btn size="sm" variant="ghost" icon="plus">Subir</Btn>} noPad>
          {oc.clips.map(c => (
            <div key={c.id} className="hairline-b last:border-b-0 flex items-center gap-3 p-4 hover:bg-[var(--cream)] cursor-pointer">
      ...
      <div className="col-span-12">
        <Card label={`Resumen del coach · ${file.coach} · 02 May 2026`} title="Reporte mensual" dark>
          <div className="grid md:grid-cols-2 gap-6 text-white/85 text-[14px] leading-relaxed">
            <p>Daniela tuvo un mes muy productivo. La final del AMTP Puebla fue una excelente lectura de su nivel: ganó dos sets contra una jugadora top y solo perdió porque su patrón se rompió en momentos clave. Su saque sigue mejorando: el primero llegó al 64% promedio del mes, con velocidades medias arriba de 150 km/h.</p>
            <p>Lo que sigue es la consolidación: trabajar el segundo servicio bajo presión y la volea de revés. En lo táctico es muy disciplinada; lo que necesita es la fortaleza mental para mantener su plan cuando la cuenta se cierra.</p>
          </div>
        </Card>
      </div>
    </div>
    ...
    <div className="grid grid-cols-12 gap-4">
      ...
      <div className="col-span-12">
        <Card label="Timeline de lesiones" title="Historial · sin lesiones activas"
          ...
          <div className="relative">
            <div className="absolute left-3 top-2 bottom-2 w-px" style={{ background: 'var(--line-strong)' }} />
...
function TabMental({ file }) {
  ...
  return (
    <div className="grid grid-cols-12 gap-4">
      <div className="col-span-12 lg:col-span-7">
        <Card label="Métricas psicología · Dra. Solís" title="Mental performance"
          ...
          <div className="space-y-6">
            {m.metrics.map((mt, i) => (
              <div key={i} className="grid grid-cols-12 gap-4 items-start hairline-b pb-5 last:border-b-0 last:pb-0">
                ...
                <div className="col-span-12 md:col-span-3"><ScoreBar value={mt.v} /></div>
      ...
      <div className="col-span-12 lg:col-span-5">
        <Card label="Sesiones recientes" title="Bitácora psicología" noPad>
          ...
          <div className="p-4 hairline-t" style={{ background: 'var(--cream)' }}>
            <div className="flex items-center gap-2 text-[12px]">
              <Icon name="quote" size={14} style={{ color: 'var(--accent)' }} />
              <span className="italic text-[var(--ink-soft)]">
                &ldquo;Es coachable. Lo más importante es la coherencia entre lo que dice y lo que hace en cancha.&rdquo;
              </span>
            </div>
        ...
        <Card label="Ética de trabajo · 1–5" title="Work ethic">
          <div className="flex items-center gap-6">
            ...
            <div className="flex-1">
              ...
              <div className="mt-3 text-[10px] font-mono text-[var(--ink-mute)] uppercase">Owner · {file.coach}</div>
            </div>
          </div>
        </Card>
        <Card label="Liderazgo y mentoría" title="Evidencia">
          <p className="text-[13px] leading-relaxed">{c.leadership}</p>
        </Card>
      </div>

      <div className="col-span-12 lg:col-span-7">
        <Card label="Registro de conducta" title="Conduct log"
              action={<Btn size="sm" variant="ghost" icon="plus">Agregar</Btn>} noPad>
          {c.conduct.map((e, i) => (
          ...
          <div className="px-5 py-3 hairline-t text-[11px] font-mono text-[var(--ink-mute)] flex items-center justify-between">
            ...
            <span style={{ color: 'var(--good)' }} className="font-bold">★ ★ ★ ★ ★</span>
          </div>
        </Card>
      </div>
    </div>
    ...
    <div className="grid grid-cols-12 gap-4">
        ...
        <Card dark label="PTF más reciente · en primera persona"
          ...
          <div className="relative pl-6 border-l-2" style={{ borderColor: 'var(--accent)' }}>
            <p className="font-display italic text-[24px] leading-[1.25] text-white">
              &ldquo;{v.latestPTF.reflection}&rdquo;
            </p>
          </div>
          <div className="mt-8 grid md:grid-cols-2 gap-4">
            <div className="p-4" style={{ background: 'rgba(255,255,255,.05)' }}>
      ...
      <div className="col-span-12 lg:col-span-4">
        <Card label="Auto-evaluación" title="Cómo se ve a sí misma">
                ...
                <div key={i}>
                  <div className="flex items-baseline justify-between mb-2">
                    ...
                    <div className="font-mono text-[10px] text-[var(--ink-mute)]">
                      vs coach <span className="text-[var(--ink)] font-bold">{coach?.score.toFixed(1) ?? '—'}</span>
                    </div>
                  </div>
                  <div className="relative h-2.5">
                    ...
                    {coach && (
                      <div className="absolute top-0 bottom-0"
                           style={{ left: 0, width: `${(coach.score / 5) * 100}%`, background: 'var(--accent)' }} />
                    )}
          ...
          <div className="hairline-t mt-5 pt-3 flex items-center justify-between text-[10px]">
            <span className="flex items-center gap-1.5">
              <span className="w-3 h-1.5 inline-block" style={{ background: 'var(--accent)' }} /> Coach
            </span>
            <span className="flex items-center gap-1.5">
              <span className="w-px h-3 inline-block" style={{ background: 'var(--ink)' }} /> Self
            </span>
          </div>
        </Card>
      </div>
          ...
          <ul className="space-y-3">
            {v.goals.map((g, i) => (
              <li key={i} className="flex items-start gap-3 hairline-b pb-3 last:border-b-0 last:pb-0">
                <div className="w-6 h-6 flex items-center justify-center shrink-0 font-mono text-[10px] font-bold text-white"
      ...
      <div className="col-span-12 lg:col-span-5">
        <Card label="PTFs · histórico" title="Reflexiones por torneo"
              ...
              noPad>
          {file.tournaments.slice(0, 5).map(t => (
    ...
    <div className="grid grid-cols-12 gap-4">
      <div className="col-span-12">
        <Card label="Historial de torneos" title="Tournaments · 2024–2026"
              action={
                <div className="flex items-center gap-1">
                  {['Todos', 'AMTP', 'ITF', 'UTR'].map((p, i) => (
                    <button key={p}
                            className={`px-2.5 h-7 text-[11px] font-mono uppercase ${i === 0 ? 'text-white' : 'text-[var(--ink-mute)]'}`}
                            style={i === 0 ? { background: 'var(--ink)' } : {}}>
                      {p}
                    </button>
                  ))}
              ...
              noPad>
          <table className="w-full text-[12px]">
            <thead className="text-[10px] eyebrow text-[var(--ink-mute)]" style={{ background: 'var(--cream)' }}>
              <tr>
          ...
          <table className="w-full text-[12px]">
            ...
            <tbody>
              {file.documents.map((d, i) => (
                <tr key={i} className="hairline-t hover:bg-[var(--cream)] transition">
                  <td className="px-5 py-3">
                  ...
                  <td className="px-5 py-3 text-right">
                    <button className="text-[10px] font-mono uppercase hover:underline" style={{ color: 'var(--accent)' }}>
                      Descargar
                    </button>
                  </td>
                </tr>
...
// ─── Talent Card Modal ────────────────────────────────────────────────────────
function TalentCardModal({ file, onClose }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(14,36,25,.85)' }}>
      <div className="w-full max-w-2xl bg-[var(--paper)] hairline relative">
        <div className="px-6 py-4 hairline-b flex items-center justify-between">
          <div>
            <Eyebrow className="mb-1">Talent Card</Eyebrow>
            <div className="font-display font-extrabold text-[20px]">{file.nombre}</div>
          </div>
        ...
        <div className="p-6">
          <div className="grid grid-cols-3 gap-px hairline mb-6">
            <HeadlineMetric label="UTR" v={file.utr.now.toFixed(1)} delta={`+${file.utr.delta12.toFixed(1)}`} accent />
            <HeadlineMetric label="AMTP" v={`#${file.amtp.now}`} delta="U16" />
            <HeadlineMetric label="ITF SJR" v={`#${file.itf.sjr}`} delta={`${file.itf.entrants}`} />
          </div>
          <div className="grid grid-cols-5 gap-4">
            {file.dimensions.map(d => (
              <div key={d.key} className="text-center">
                <ScoreRing value={d.score} size={64} label={d.label} />
              </div>
            ))}
          </div>
          <div className="mt-6 flex justify-end gap-2">
            <Btn variant="outline" size="md" icon="print">PDF</Btn>
            <Btn variant="primary" size="md" icon="share">Compartir</Btn>
          </div>
        </div>
      </div>
    </div>
